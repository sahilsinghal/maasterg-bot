...
const noopStore = {
  getOrCreate() {}, updateProfile() {}, recordInteraction() {}, stats() { return { totalPeople: 0, withName: 0, withCity: 0 }; }
};
...
class Engine {
  ...
  handle(session, rawInput, jid) {
    ...
    this.store.recordInteraction(jid, { text: raw, stage: session.stage, nodeId: this.currentNode(session)?.id });

    // First contact
    if (!session.welcomed) {
      session.welcomed = true;
      session.stage = 'language';
      this.store.getOrCreate(jid);
    ...
    if (chosen) {
      this.store.recordInteraction(jid, { text: raw, stage: 'browse', nodeId: chosen.id });
